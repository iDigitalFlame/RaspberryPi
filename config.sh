...
/usr/bin/printf ' exit 0\n    fi\n    pacman -Syu --noconfirm\nelse\n    env SYSUPDATE=1 pacman -' >> "${SETUP_DIRECTORY}/bin/sysupdate"
/usr/bin/printf 'Syu\nfi\n\n[ ${SYSCONFIG_SECURE:-0} -eq 1 ] && mount -o rw,remount /\n[ -e "/etc' >> "${SETUP_DIRECTORY}/bin/sysupdate"
...
/usr/bin/printf 'if [ ${SYSCONFIG_SECURE:-0} -eq 1 ]; then\n    mount -o ro,remount / 2> /dev/nul' >> "${SETUP_DIRECTORY}/bin/sysupdate"
...
/usr/bin/printf '      yes\nKerberosAuthentication          no\nHostbasedAuthentication         n' >> "${SETUP_DIRECTORY}/etc/ssh/sshd_config"
...
/usr/bin/printf '#!/usr/bin/bash\n\nln -sT "/etc/sysless" "/etc/syslesskey" 2> /dev/null\nln -sT ' >> "${SETUP_DIRECTORY}/etc/syscheck.d/lesskey.sh"
/usr/bin/printf '"/etc/sysless" "/usr/local/etc/syslesskey" 2> /dev/null\n\nchmod 0444 "/etc/sysl' >> "${SETUP_DIRECTORY}/etc/syscheck.d/lesskey.sh"
